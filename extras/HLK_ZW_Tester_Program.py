"""
HLK-ZW Fingerprint Module Tester  [PyQt6]
...
FINGERPRINT_READINFPAGE    = 0x16
FINGERPRINT_LEDON          = 0x50
FINGERPRINT_LEDOFF         = 0x51

# Capacity → possible HLK-ZW series families (all share the EF-01 UART protocol).
# Multiple models can report the same capacity so this is always an inference.
_VARIANT_MAP = {
...
class StorageMapWidget(QWidget):
    _COLS = 25  # fixed column count; rows grow with capacity

    def __init__(self, parent=None):
        super().__init__(parent)
        self._states = [False] * 50
        self.setMinimumWidth(300)
        self._update_height(50)

    def set_capacity(self, capacity):
        self._states = [False] * capacity
        self._update_height(capacity)
        self.update()

    def _update_height(self, n):
        rows = max(1, (n + self._COLS - 1) // self._COLS)
        self.setMinimumHeight(rows * 22 + 10)
    ...
    def paintEvent(self, _event):
        ...
        for i in range(n):
            ...
            cw = max(1, cell - pad)
            ch = max(1, cell - pad)
            fill = QColor("#00cc44") if self._states[i] else QColor("#333333")
...
class App(QMainWindow):
    ...
    def __init__(self):
        ...
        self.setWindowTitle("HLK-ZW Fingerprint Tester")
    ...
    def _build_device_tab(self, tabs):
        ...
        for text, slot in [
            ...
            ("Dump Info Page",       self.cmd_dump_info_page),
        ]:
        ...
        info_grid.addWidget(self.dev_variant_lbl, 1, 1)
        info_grid.addWidget(self.dev_params_lbl,  2, 1)
        info_grid.addWidget(self.dev_count_lbl,   3, 1)
        ...
        opt_row = QHBoxLayout()
        self._enroll_target_lbl = QLabel("Target ID (0–49):")
        opt_row.addWidget(self._enroll_target_lbl)
    ...
    def _build_settings_tab(self, tabs):
        ...
        self.theme_cb.currentTextChanged.connect(
            lambda t: (apply_theme(QApplication.instance(), t.lower()),
                       QSettings("GavinnnTann", "HLK-ZW-Tester").setValue("theme", t.lower())))
    ...
    def toggle_connect(self):
        if self.ser and self.ser.is_open:
            ...
            self.conn_lbl.setStyleSheet("color: red; font-weight: bold;")
            for lbl in (self.dev_status_lbl, self.dev_variant_lbl,
                        self.dev_params_lbl, self.dev_count_lbl):
                lbl.setText("—"); lbl.setStyleSheet("")
            self._apply_capacity(50)
    ...
    def _apply_capacity(self, cap):
        """Update every capacity-dependent widget. Must be called on the main thread."""
        self._capacity   = cap
        self._map_states = [False] * cap
        for spin in (self.chk_id, self.del_id, self.del_first, self.del_last,
                     self.enroll_id, self.export_id_spin, self.import_id_spin):
            cur = spin.value()
            spin.setRange(0, cap - 1)
            spin.setValue(min(cur, cap - 1))
        self._enroll_target_lbl.setText(f"Target ID (0–{cap - 1}):")
        self.map_widget.set_capacity(cap)
    ...
    def _auto_connect_query(self):
        def _run():
            time.sleep(0.2)
            capacity = 50  # default until ReadSysParam confirms otherwise
            ...
            if cc == 0x00 and data and len(data) >= 16:
                ...
                self._ui(lambda c=capacity: self._apply_capacity(c))

                family      = _VARIANT_MAP.get(capacity, "EF-01 compatible")
                variant_txt = f"{family}  (cap={capacity})"
                self._ui(lambda t=variant_txt: self.dev_variant_lbl.setText(t))

            bytes_needed = (capacity + 7) // 8
            cc, data = self.send_recv(FINGERPRINT_READ_INDEX, bytes([0x00]))
            if cc == 0x00 and data and len(data) >= bytes_needed:
                states = []
    ...
    def cmd_dump_info_page(self):
        """Dump the raw 512-byte PS_ReadINFpage (0x16) response to the log.
        Each row shows: offset (hex)  16 hex bytes  ASCII representation.
        This lets you manually locate where your firmware stores the device strings."""
        def _run():
            if not (self.ser and self.ser.is_open):
                self.log_msg("ERROR: Not connected"); return
            pkt = build_packet(FINGERPRINT_READINFPAGE)
            raw = None
            with self.lock:
                try:
                    self.ser.reset_input_buffer()
                    ...
                    if len(buf) < 9:
                        self.log_msg("[Dump] Timeout: no ACK"); return
                    length = struct.unpack('>H', buf[7:9])[0]
                    total  = 9 + length
                    while len(buf) < total and time.time() < deadline:
                    ...
                    if cc != 0x00:
                        self.log_msg(f"[Dump] Rejected: CC=0x{cc:02X}"); return
                    raw = self._recv_packets(timeout=5.0, max_bytes=512)
                except Exception as e:
                    self.log_msg(f"[Dump] Error: {e}"); return

            if not raw:
                self.log_msg("[Dump] No data received"); return

            self.log_msg(f"[Dump] Info page — {len(raw)} bytes:")
            self.log_msg("       00 01 02 03 04 05 06 07  08 09 0A 0B 0C 0D 0E 0F   ASCII")
            for row in range(0, len(raw), 16):
                ...
                self.log_msg(f"  {row:04X}  {hex_lo:<23}  {hex_hi:<23}   {asc}")

        threading.Thread(target=_run, daemon=True).start()

    def cmd_storage_map(self):
        cap          = self._capacity
        bytes_needed = (cap + 7) // 8
        cc, data = self.send_recv(FINGERPRINT_READ_INDEX, bytes([0x00]))
        if cc == 0x00 and data and len(data) >= bytes_needed:
            states = []
            for i in range(cap):
    ...
    def _led_send(self, func, start, end, cycles, label):
        ...
        if cc == 0x00:
            ...
            # AURALEDCONFIG (0x3C) is ZW1xx-only; fall back to simple on/off for passive variants
            if func == 4:  # FP_LED_FUNC_OFF
                cc2, _ = self.send_recv(FINGERPRINT_LEDOFF)
                if cc2 == 0x00:
                    self.log_msg("LED → Off (simple)")
            else:
                cc2, _ = self.send_recv(FINGERPRINT_LEDON)
                if cc2 == 0x00:
                    self.log_msg(f"LED → On (simple — RGB not supported on this variant)")
...
if __name__ == "__main__":
    ...
    saved    = QSettings("GavinnnTann", "HLK-ZW-Tester").value("theme", "")
